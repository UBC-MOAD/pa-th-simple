...
def plot_profiles(zmin,zmax,nz,nx,Conc):
    ...
    plt.subplot(1,2,1)
    center = np.int(nx/2)
    plt.plot (Conc[:,center-2],dz*np.arange(20),'bo-',label="Downwelling Side")
    plt.plot (Conc[:,center],dz*np.arange(20),'rx-',label="Center")
    plt.plot (Conc[:,center+2],dz*np.arange(20),'k+-',label="Upwelling Side")
    plt.title("Concentration")
    plt.ylim (zmax,zmin)
    plt.legend(loc='center left')
    plt.subplot(1,2,2)
    plt.plot (1e4*(Conc[:,center-2]-Conc[:,center+2]),dz*np.arange(20),'gs-',label = "Downwelling-Upwelling")
    plt.plot (1e4*(Conc[:,center]-Conc[:,center+2]),dz*np.arange(20),'m^-',label = "Center-Upwelling")
